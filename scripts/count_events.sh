...
dir=$(dirname ${file})
for s3file in $(mc find S3/eictest/EPIC/EVGEN/${dir} -name ${file}.${ext}) ; do

  # if nevents not known
  if [[ "${nevents}" == 0 ]] ; then
    # count events
    nevents=$(mc cat ${s3file} | ${GUNZIP[@]} | grep ^E | wc -l)
  fi
  ...
  if [ -z "${n_lines_per_event}" ] ; then
    ...
    n_events_10k=$(mc cat ${s3file} | ${GUNZIP[@]} | head -n 10000 | grep ^E | wc -l)
